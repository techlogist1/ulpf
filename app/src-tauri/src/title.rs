...
// 2 pending`, once a second from /api/metrics and /api/pending. While the engine is
// starting or down, `start` and `fail` own the title.
...
pub(crate) fn title_loop(app: &AppHandle) {
    loop {
        ...
        if let (Some(events), Some(pending)) = (events, pending) {
            set_title(app, &format!("ULPF · engine ok · {} events · {pending} pending", commas(events)));
        }
    }
}
