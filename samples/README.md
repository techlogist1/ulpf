...
| `openvpn.log` | 29-34 — `corpus/real/openvpn/*` (Azure-Sentinel and DFIRArtifactMuseum, both MIT): the `--syslog` wire form, tag with and without `[pid]`, peer prefix in all three shapes plus a daemon line with none. |
...
for what exists for each and why it was not promoted.
...
| `openvpn.log` | OpenVPN 2.6 manual and source message strings | CRLF, non-UTF-8 bytes in a common name (two lines), truncated line, daemon lines with no peer |
