...
`pfsense_filterlog.log`, `sophos_xg.log`, `suricata_eve.log`, `windows_event.log` — see
`corpus/README.md` for what exists for each and why it was not promoted (the Windows
lines 1, 4 and 7 are Microsoft's own documented examples, which is the nearest thing to
a capture without a Windows host). `cef.log`, `leef.log` and `cloudtrail.log` (added
2026-09-06) are built from the specifications' own example lines and records, extended
with the dictionary keys the specifications define; no capture.
...
| `windows_event.log` | Windows Event Log XML: the Event XML examples on the 4624, 4625 and 4720 Security auditing pages verbatim (lines 1, 4, 7) plus events built from the same pages and the Sysmon page's event 1, 3, 11 descriptions (learn.microsoft.com, read 2026-09-06). Event Viewer's multi-line rendering is collapsed to one event per line, the form Windows Event Forwarding and the agents send; endpoint telemetry, not a perimeter device (D75) | the `<?xml ...?>` declaration prefix, double- and single-quoted attributes (`wevtutil`'s form), CRLF, a collector's pretty-printing joined with spaces, `&amp;` `&lt;` `&gt;` `&quot;` `&#174;` entities in values, a trailing space the device keeps (`User32 `), an empty `TargetDomainName`, IPv6 endpoints, a non-ASCII UTF-8 file name, hex and decimal process ids |
