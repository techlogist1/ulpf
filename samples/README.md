...
| `pfsense_filterlog.log` | Netgate "Raw Filter Log Format" | RFC 3164 and RFC 5424 framings, a row ending in a delimiter, a row cut inside the common columns, non-UTF-8 byte in the interface name, no-`<pri>` line, TCP/UDP/ICMP/ICMPv6/CARP/GRE tails |
...
| `sonicwall.log` | SonicOS Log Events Reference Guide | no syslog header (as the device sends it) and a relayed header, single-quoted `appName`, four-part `src`, `time` without a zone, non-UTF-8 byte in `usr`, truncated line |
