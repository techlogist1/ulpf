...
ulpf demo [--auto] [--check] [--reset] [--dir demo] [--listen 127.0.0.1:7878] [--syslog 127.0.0.1:5514] [--repo .]   # the PROGRESS demo played from the binary (D67); scripts/demo.sh wraps it
```
...
beside it, and `FILE.pivot` when the entity index is on (`--pivot on`; the default in `serve`).
...
- `docs/coverage.md` — per-file coverage table over samples and corpus, regenerated by
  `scripts/coverage.sh`.
- `docs/manual-test.md` — the hand-test checklist for the CLI and the desktop app on Windows and
  macOS, with the expected observation and its source beside every step; nothing in it has been
  executed on the Windows machine yet.
