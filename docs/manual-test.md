...
Every expectation here was read out of the tree, not remembered. Where nothing in the tree
...
first run and no proposal is ever raised. Two gates catch it. Both sidecar scripts
(`app/scripts/sidecar.sh` and `app/scripts/sidecar.ps1`) test each `parsers/*.toml` for a line matching
...
copy skips such a file rather than copying it (`app/scripts/sidecar.sh`
lines 20-35, and `sidecar.ps1` lines 11-18; `prepare()` and `is_generated()` in
`app/src-tauri/src/lib.rs`). Either way `ulpf demo --reset` removes
them (`purge_generated` in `crates/ulpf/src/demo.rs`) — that is the command to run before a
bundle. So: approve nothing from the CLI before the installers are built
...
| M3 | `./target/release/ulpf check --pending pending` | One `parser  <name> <vendor> <product> (N subs, M timestamp candidates)` line per definition, one `mapping` line per schema, then `16 parsers, 2 mappings loaded; 0 problems`; exit 0 (`Cmd::Check` in `cli.rs`; PROGRESS, demo section). A non-zero exit means problems, and the `ERROR` lines above it name the path. |
| M4 | `./target/release/ulpf run samples/*.log --store $S/store --output $S/out.jsonl` | The counter block, with the stage counts the README's Run-it block carries for the sixteen samples (README, "Run it"; that block is pasted from the binary and is the one place the numbers live). `samples` instead of `samples/*.log` gives the same event counts plus `2 excluded` and two `excluded:` lines (D83). |
| | | Also expected there: `parse_failed by reason: pattern_no_match 1, invalid_json 1`, and `pending: 0 proposals awaiting review`. The `events/s` on the header line is a few hundred events in a few milliseconds — startup noise, not a throughput measurement, and the README says so in the same place. Do not report it as a number. |
| M5 | `./target/release/ulpf verify --store $S/store` | `store <hex> genesis <hex>`, then `verified N records, 0 corrupt` with N the README block's event count, then `chain ok (head <hex>)`; exit 0 (`Cmd::Verify` in `cli.rs`). |
...
- **`--output NUL` is a sink.** `NUL`, `\\.\NUL` and `\\?\NUL` (and `/dev/null` on the
  other platforms) leave no `.meta.json` and no pivot index beside them (D82); wherever a macOS
  command says `--output /dev/null`, `NUL` is the equivalent.
...
| A4 | Check the first-run copy | `<data>/parsers/` holds the sixteen bundled definitions and `<data>/mappings/` holds two. Nothing named `*_inferred.toml` is there. They are seeded only when the directory holds no TOML, so an approved or edited parser is never overwritten on a later launch (`prepare()` in `lib.rs`; D73). | List the directory. Before lane 7D a successful copy writes **nothing** to `engine.log` — the file opens with the engine's own first line — so the directory listing is the whole check. **Now** there is still no line for the files it did copy; the one line it can add is a skip, and only if there was something to skip: `shell: N generated definition(s) not copied into <path>: <files>`, written both to `engine.log` and to the app's stderr with a `[shell] ` prefix (`prepare()` and `is_generated()` in `app/src-tauri/src/lib.rs`). On a clean checkout there is nothing to skip and the line is absent; that is a pass, not a missing line. |
...
| A13 | `File > Add folder…` (Cmd/Ctrl+Shift+O), pick `samples` | A native folder picker titled `Add a folder of logs to ULPF`, then `Added 17 files to the watch folder: samples`. **The count and the names count different things**: the count is every regular file found by walking the tree (seventeen — the sixteen `*.log` plus `samples/README.md`, which the watch scan then keeps out under D83's default excludes and counts in `files_excluded`), the name list is one entry per item you picked, so picking one folder gives one name and no `, …` however many files came with it. Folders keep their structure; only regular files are copied, and symlinks, sockets and devices are skipped (`ingest_paths` and `copy_tree` in `ingest.rs`). | The notice's count, then Live's source list. Live gains sixteen sources; the README is not one of them (`excluded` in the counter block, D83). |
...
| A36 | **Force kill.** Windows: Task Manager > `ulpf-app.exe` > End task. macOS: `kill -9` the app process | This skips the clean quit path on both platforms. **Before lane 7D** the engine can survive it on Windows: CI's own smoke run found `ulpf.exe` outliving a `Stop-Process` of the window and prints that as the expected result there — `orphan: ulpf.exe pid N outlived a Stop-Process of the window (expected on Windows; the tray's Quit is what kills it)` — with the tray's Quit left as the human check (`app/scripts/smoke-windows.ps1`; D89). **Now** the sidecar is put in a job object carrying `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` whose handle the app holds for its whole life, so the kernel terminates `ulpf.exe` when the app's handles close, however it died, and CI turns the note into an assertion: no `ulpf.exe` five seconds after the force kill or the job fails (`adopt()` in `app/src-tauri/src/job.rs`, called right after the spawn in `app/src-tauri/src/lib.rs`; `app/README.md`, "Platform differences"). On **macOS** the tree now says what happens instead of claiming immunity: `job.rs`'s comment records that Unix does not kill a child when its parent dies and a SIGKILLed parent runs no code to kill it either, so `kill -9` of the app leaves `ulpf serve` running and holding the store (measured on a Mac in this session). The clean-quit path is what stops it there; a force quit leaves it until the next launch, which names the orphan and offers to stop it and start again (`app/src-tauri/src/job.rs`'s comment, `start()` in `lib.rs`, `app/README.md` "Platform differences") — that recovery is A38. There is still no automated macOS force-kill test, so record what you saw rather than confirming a claim. | Windows: `Get-Process ulpf`. macOS: `pgrep -l ulpf`. Record the answer on each machine rather than confirming a claim; then clean up by hand and relaunch, which is A38. |
...
| A38 | **Store held by another writer.** Start `ulpf serve` by hand against `<data>/store` (give it `--listen 127.0.0.1:7979` so it does not also collide with the demo's port), then launch the app | **Before lane 7D**: the sidecar spawns, is refused by its own writer and exits, so the generic sentence is what you get — `The engine stopped (exit 2). Its last words: ulpf: opening store <path>: store <path> is in use by another process` then `The whole of its output is in <data>/engine.log`, and the title says `ULPF · engine down (exit 2)`, quoting the code because nothing on main reads the refusal (the `Terminated` arm, `fail()` and `down()` in `lib.rs`; `exit(2)` in `crates/ulpf/src/main.rs`; the `in_use` text and the `opening store` context in `crates/ulpf-store/src/store.rs` and `crates/ulpf/src/engine.rs`). **Now**: the sidecar still spawns and still dies first — the holder is looked up in the `Terminated` arm, not before the spawn — but the store-in-use case is pulled out of the generic arm and gets its own sentence and a button: `The engine's store at <path> is held by ulpf (pid N). Stop it and start again?` then `The whole of its output is in <data>/engine.log`, with one button reading `Stop it and start again` that force-kills that pid and retries through the ordinary start path. If the holder exited between the refusal and the lookup the sentence is `... is held by another writer that is no longer running. Start again?` instead, and there is nothing to press but the retry. The holder is found by scanning process command lines for a `ulpf` naming the same `--store` path, because the lock is SQLite's and carries no pid (`locked()` in `app/src-tauri/src/lib.rs`, `find()` and `kill()` in `app/src-tauri/src/holder.rs`, the `#act` button in `app/dist/index.html`). Title there: `ULPF · engine down (store in use)`. Two later refinements are in as well: the button no longer refuses when the holder exited between the refusal and the click (a failed kill is a notice and the start goes ahead), and a launch that finds a stale `server.url` with a live `ulpf` on the store goes straight to this page instead of starting a second engine to be refused (`stop_holder()` and the pre-flight in `start()`, `app/src-tauri/src/lib.rs`). |
